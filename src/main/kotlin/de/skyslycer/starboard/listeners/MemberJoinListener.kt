...
class MemberJoinListener(private val guilds: Guilds, private val logger: Logger) : ListenerAdapter() {
    override fun onGuildMemberJoin(event: GuildMemberJoinEvent) {
        if (!guilds.containsKey(event.guild.idLong)) {
            logger.info("Creating new guild settings for guild ${event.guild.id}")
            val channel: TextChannel? = event.guild.textChannels.stream().filter { it is TextChannel }.collect(Collectors.toList()).toTypedArray()[0]
        ...
        if (!matcher.find()) {
            try {
                ...
                logger.info("Name of the joined member doesn't match with the given regex. Setting to ${guildSettings.invalidName}. " +
                ...
                logger.info(
                    "Name of the joined member doesn't match with the given regex. But the member has a higher/the highest role on the server. Aborting. [Guild:${event.guild.id}]")
            }
        } else {
